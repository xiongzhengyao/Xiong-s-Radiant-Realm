...
echo "==> Rolling back ${REMOTE}:${DEPLOY_PATH} to ${ROLLBACK_REF}"

ssh -p "${DEPLOY_PORT}" "${REMOTE}" \
  "DEPLOY_PATH='${DEPLOY_PATH}' DEPLOY_BRANCH='${DEPLOY_BRANCH}' DEPLOY_APP_NAME='${DEPLOY_APP_NAME}' DEPLOY_APP_PORT='${DEPLOY_APP_PORT}' ROLLBACK_REF='${ROLLBACK_REF}' bash -s" <<'EOF'
set -euo pipefail

echo "==> [remote] Start rollback: $(date)"
...
echo "==> [remote] Fetch and switch branch"
git fetch --all --prune
git checkout "${DEPLOY_BRANCH}"
git pull --ff-only origin "${DEPLOY_BRANCH}"

echo "==> [remote] Checkout rollback ref: ${ROLLBACK_REF}"
...
echo "==> [remote] Rollback completed successfully: $(date)"
EOF
